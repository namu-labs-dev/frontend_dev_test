import React, { useState } from "react";

const tabs = ["Check In", "Quiz", "Gacha"];

type Props = {
  tabs: string[];
};
const DailyEarnMenuAtom = (props: Props) => {
  const [activeTab, setActiveTab] = useState(props.tabs[0] ?? "");

  const handleTabClick = (tab: string) => {
    setActiveTab(tab); // Set the clicked tab as active
  };

  return (
    <div className='ml-5'>
      <div className='flex max-w-[7rem] translate-x-[-4px] translate-y-[-4px] flex-col gap-y-2 rounded-md border-2 border-black bg-white px-[1%] py-[1%] text-center text-sm  text-black shadow-[4px_4px_0px_black] transition-all duration-300 hover:rounded-md active:translate-x-[0px] active:translate-y-[0px] active:rounded-2xl  active:shadow-none '>
        {props.tabs.map((tab, index) => (
          <button
            key={index}
            className={`my-1 inline-block rounded-lg ${activeTab === tab && "bg-black px-1 py-1 text-[#00FF00]"}`}
            onClick={() => handleTabClick(tab)} // Set active tab on click
          >
            {tab}
          </button>
        ))}
      </div>
    </div>
  );
};

export default DailyEarnMenuAtom;
