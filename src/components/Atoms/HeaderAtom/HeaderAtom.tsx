...
import { SettingsModal } from "~/components/Components/SettingsModal/SettingsModal";
  ...
  return (
    ...
    >
      ...
      {props.onClickLeftIcon && (
        ...
        >
          <SVGAtom
            iconName={`${props.iconName ? props.iconName : "arrowLeft"}`}
            width={20}
            height={20}
      ...
      {props.onClickRightIcon && (
        <div
          ...
          onClick={() => {
            if (props.modalProps) {
              props.modalProps.modalProps.setModalOpen(true);
            } else {
        ...
        >
          <SettingOutlined
            className={`${props.darkTheme ? "!text-white" : "!text-gray-header"} cursor-pointer text-[20px]`}
          />
