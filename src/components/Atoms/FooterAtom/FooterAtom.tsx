...
const icons: Array<{ iconName: 'dollar' | 'message' | 'wallet' | 'history', width: number, height: number, color: string, special?: boolean }> = [
  ...
  return (
    <div className='flex h-full items-center justify-between bg-[#1F1F1F] px-16 py-5 text-white'>
      {icons.map((icon, index) => (
        <div
          key={index}
          className={`z-[1] flex w-[50px] cursor-pointer items-center justify-center ${icon.special ? 'relative pt-2' : ''}`}
        >
          {icon.special && (
